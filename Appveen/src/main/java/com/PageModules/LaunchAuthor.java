...
public class LaunchAuthor extends ObjectLoginAuthor {
	...
	public void getCaptionColorForDataService() {
		...
		String expectedTextValue="rgba(108, 117, 125, 1)";
		boolean result=expectedTextValue.equalsIgnoreCase(expectedTextValue);
		System.out.println("The expected result is : "+result);
	}
	public void getCurrentRecordNumber() {
		String text=getText(login.recordCount);
		System.out.println("The current number of Records are : "+text);
	}
	public void getCurrentAttributes() {
		String text=getText(attributeValue);
		System.out.println("The number of current attributes are : "+text);
	}
